...
- **Spec 2 (secretary web shavings page) — BUILT 2026-07-26 on `feature/shavings-page-redesign` (commit `e8e69ff`, NOT merged, NOT deployed).** Replaces the neutral placeholder with the order-list redesign: ranch⇄status grouping (URL `?group=`/`?ranch=`/`?status=` via `useSearchParams`), derived-status chips, SLA needs-attention (`SHAVINGS_SLA_THRESHOLD_HOURS=3`, Rule A unclaimed>3h since created / Rule B claimed-undelivered>3h since seen), add-order modal with mandatory ranch (#32). New web files: `services/shavingsOrderService.js` (restored), `utils/shavings{Status,Sla,Grouping}.utils.js`, `hooks/secretary/useCompetitionShavingsPage.js`, `components/secretary/shavings/*`, `pages/secretary/CompetitionShavingsPage.jsx`. Verified via a temp dev-harness against live comp-7 data (harness removed). **Pending:** Oren's Hebrew-label final edits, mobile-admin shavings smoke-test (reads #176), browser screenshots (pane was closed during build), and merge. Live gotcha found: comp-7's 4 Delivered orders (38/39/40/46) DO carry real `deliveryphotourl` → "delivered without photo" marker is absent in live data; and a timestamptz serialized by pg console as `+00` is NOT JS-`Date`-parseable (the real API emits ISO `Z`/offset, which is).
