...
- **iss 40 is FIXED** (commit `a3ade7b`, branch `feature/schedule-view`, 2026-07-21). `computeDaySchedule` previously threaded a single cursor across every class in a day, serializing tied classes — a 5-class tie was charged five durations plus four gaps instead of one and one. It now groups a day into `orderInDay` positions via `groupClassesByPosition` and advances once per position: position duration = MAX of its classes' durations, one between-class gap per position BOUNDARY, and every tied class shares the position's start AND finish in `perClass`. `durationMinutes` deliberately stays each class's OWN run length, so for the short class in a tie it is less than `finish - start` — that is not a bug, and a test pins it.
...
- **`minutesperentry` live values:** reining 5/5, cutting 2.5/2.5, extreme 5/7. The switcher's average option uses the mean, so extreme = 6; for reining and cutting min and max are equal, so all three switcher states show the same number. The switcher's first label is **ממוצע** (was רגיל, renamed 2026-07-21); the prediction-column switcher is likewise **ממוצע / טווח** (was מספר / טווח).
...
- **Open at time of writing (2026-07-21):** **nothing on this branch has been verified against the running app with real data** — the three-view structure was verified through a temporary dev harness with mock props, and everything since through tests and builds only (Claude Code cannot authenticate; see Working Conventions). The reining flattening model is not yet implemented (gap is still applied uniformly to every field). The 06:00 suggestion floor clamps silently with no text when the clamped suggestion no longer resolves the overage. The `classDateTime` timestamptz round-trip through the apply button is unverified. **iss 45** (day origin ignores a real `starttime` when null-order classes sort ahead of it) is filed and unfixed.
...
- **THE NOTIFICATION PIPELINE DOES NOT EXIST.** `RideOnServer/BL/Notification.cs` is a bare 16-line POCO with no DAL, no BL methods, no controller and no web service, and there is **no push transport anywhere** — no Expo notifications, no push tokens, no Firebase, in either the mobile app or the server. The button is a demo, carries a permanent "הדגמה בלבד" notice, and calls nothing. Built at Oren's explicit request to demo the flow.
