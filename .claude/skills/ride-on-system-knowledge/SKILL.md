...
- **Procs are LIVE-ONLY — none of these four exist in the repo.** Read them live via Supabase MCP; do not trust `RideOnDB/` for this feature:
  - `usp_getsecretarycompetitionchangerequests(p_competitionid, p_ranchid, p_status='Pending')` — UNION of entry+product requests. Returns **20 PascalCase columns** (`RequestId, RequestSource, RequestType, CompetitionId, CompetitionName, RequestDate, RequestedByPersonId, RequestedByName, EntityType, EntityName, BeforeText, AfterText, Status, IsCancelled, OriginalEntityId, NewEntityId, FineId, FineAmountSnapshot, AmountBefore, AmountAfter`). `BeforeText`/`AfterText` are `|`-delimited `label: value` strings (incl. a `חיוב:` money part). **No `IsPaid`/`CanAnswer` flag.** `RequestType` values are localized: `'שינוי מקצה'`/`'ביטול מקצה'` (entry), `'שינוי מוצר'`/`'ביטול מוצר'` (product).
  - `usp_gethostsecretarypendingchangerequestscount(p_ranchid)` — ranch-wide pending count (badge "בקשות ממתינות בכל תחרויות החווה"); note it is ranch-wide while the list is competition-scoped.
  - `usp_answerchangeentryrequest` / `usp_answerproductchangerequest(p_id, p_answerstatus, p_answeredbysystemuserid, p_notes)` — approve/reject. Approve mutates `billcharge` and (entry) **auto-inserts a Fine billcharge** via an `effective_fine` lookup (`EntryCancellation`/`LateRegistration`, `triggermode` Between/After).
- **Core business rule (`P0001`, verified 2026-07-27):** if a `billcharge` for the source is `chargestatus='Paid'`, the proc `RAISE`s `'Cannot answer change request for a paid ... request'` — and the guard sits **above** the approve/reject split, so it blocks **both**. A paid change/cancel request therefore becomes an **un-clearable zombie in Pending** (400 on every click). The English `P0001` text leaks straight to the UI.
- **Product decision (Oren, 2026-07-27 — pending spec, NOT built):** these requests *should not exist* → **block at mobile creation** (deferred); for the demo, the **list proc should HIDE paid Pending rows** (Pending-scoped `not exists(paid billcharge)`, keep answered history) with the **same filter mirrored in the count proc**. Deferred items in `_bmad-output/change-tracking-followups.md`.
- **Frontend cargo:** API returns consistent **PascalCase**, but the hook/table/modal defend against both cases via `getValue(item, camelKey, pascalKey)` — the camelCase half is dead code.
- **Known FUNCTIONAL bug (unfixed, in spec scope):** the `סוג בקשה` type filter (`ChangeRequestsFilters.jsx`) hardcodes `'שינוי מקצה'`/`'ביטול מקצה'` and the hook matches `RequestType` exactly → **product requests silently vanish when the type filter is used.** Fix: filter on the `IsCancelled` boolean, not the localized string.
- **Shipped polish (2026-07-27, frontend-only, HMR-clean):** action feedback moved from a top-of-page banner to the shared floating `ToastMessage`; reworded approve/reject copy; killed the duplicated `חיוב`/`סכום` money line via a money-label filter in `buildChangedFields` (table + modal, still duplicated across both files).
- **UX audit (20 findings, 2026-07-27) → SPEC.** `SPEC-change-tracking-improvements` (`_bmad-output/specs/spec-change-tracking-improvements/`) distills 15 in-scope items into 11 capabilities (CAP-1…CAP-11), all open questions resolved by Oren, ready for `bmad-quick-dev`. Companions: `backend-changes.md` (exact verified proc SQL deltas for the paid-hide filter + type-filter fix location), `hebrew-strings.md` (all approved Hebrew copy with file:line anchors), plus adopted `change-tracking-followups.md` (deferred). Handoff prompt at `_bmad-output/change-tracking-quickdev-prompt.md`.
  - **Verified live 2026-07-27 (`pg_get_functiondef`):** the "paid" definition is identical in both answer procs — `billcharge.chargestatus='Paid'` with `sourcetype='Entry'`/`sourceid=originalentryid` (entry) or `sourcetype='ProductRequest'`/`sourceid=originalprequestid` (product). CAP-1 mirrors this as a Pending-scoped `not exists(...)` in both list-proc branches AND in the count proc — pure `WHERE` addition, no column/type change, no `DROP` (20-col list shape + single `PendingCount` preserved). Answer-proc `P0001` guards stay as backstop.
  - Key resolved decisions: type filter must key off `IsCancelled` boolean (not the hardcoded entry-only `RequestType` string that hid all product requests); status translated in modal via the table's existing map (`ממתינה`/`אושרה`/`נדחתה`); `P0001`→Hebrew via the toast; **both** Approve and Reject get confirmation dialogs. All 4 procs remain live-only/uncommitted — the spec's deploy step is to finally commit them to the repo.
