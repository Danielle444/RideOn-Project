...
- **Read procs (new fields):** `usp_getworkershavingsorders` (113) + `usp_getshavingsordersforworkerbycompetition` (114) each return a trailing `ResponseTime`. Secretary read `usp_getshavingsordersforcompetitionandranch` (176) dropped `ApprovedByPersonId`/`ApprovedAt` and appended `Seen`(=responsetime)/`Delivered`(=arrivaltime)/`PrequestDatetime` (order-creation clock, the SLA source). **DEP-1 (Spec 2, applied live 2026-07-26): #176 now also appends a 14th col `DeliveryPhotoUrl`** (`so.deliveryphotourl`, appended LAST, DROP+CREATE verbatim + one column). DTO `CompetitionShavingsOrderListItem` + `ShavingsOrderDAL` read it by name; repo file `176_*.sql` synced. Backward-compatible append (all consumers read by name). The C# that reads it **shipped with the Spec 2 merge (`main` `13183fc`, deployed 2026-07-27)**, closing the DB-ahead-of-backend gap — see [[feedback_deploy_coupled_db_changes]].
- **Retired procs (dropped from live):** `usp_getpendingdeliveryapprovals`, `usp_approvedelivery`. Their `pending-approvals`/`approve-delivery` endpoints, DAL/BL methods, DTOs, and the web approval action are all gone.
- **Spec 2 (secretary web shavings page) — SHIPPED 2026-07-27, merged to `main` `13183fc`** (feat commit `e8e69ff` on `feature/shavings-page-redesign`; deployed to Render). Replaces the neutral placeholder with the order-list redesign: ranch⇄status grouping (URL `?group=`/`?ranch=`/`?status=` via `useSearchParams`), derived-status chips, SLA needs-attention (`SHAVINGS_SLA_THRESHOLD_HOURS=3`, Rule A unclaimed>3h since created / Rule B claimed-undelivered>3h since seen), add-order modal with mandatory ranch (#32). Web files: `services/shavingsOrderService.js` (restored), `utils/shavings{Status,Sla,Grouping}.utils.js`, `hooks/secretary/useCompetitionShavingsPage.js`, `components/secretary/shavings/*`, `pages/secretary/CompetitionShavingsPage.jsx`. Verified via a temp dev-harness against live comp-7 data (harness removed). **Post-merge follow-ups (non-blocking):** Oren's Hebrew-label final edits, mobile-admin shavings smoke-test (reads #176), browser screenshots (pane was closed during build). Live gotcha found: comp-7's 4 Delivered orders (38/39/40/46) DO carry real `deliveryphotourl` → "delivered without photo" marker is absent in live data; and a timestamptz serialized by pg console as `+00` is NOT JS-`Date`-parseable (the real API emits ISO `Z`/offset, which is).
...
## Change Tracking — Secretary Change/Cancel Approvals (מעקב שינויים) (SHIPPED 2026-07-27, branch feature/change-tracking-improvements)

Secretary-web feature where the host secretary approves/rejects payer-submitted **change** and **cancellation** requests for **Entries** and **Products** (stalls/shavings). Route `CompetitionChangeTrackingPage.jsx`; hook `useCompetitionChangeTrackingPage.js`; components under `components/secretary/change-tracking/`; service `changeTrackingService.js`. `SPEC-change-tracking-improvements` (11 CAPs) is **implemented** on `feature/change-tracking-improvements` (base commit `a260cf9`, **not merged/pushed** as of 2026-07-27).

- **Backend chain:** `ChangeTrackingController` → `BL/ChangeTracking.cs` → `DAL/ChangeTrackingDAL.cs`. The DAL branches by `RequestSource`: `"Entry"` → `usp_answerchangeentryrequest`, `"Product"` → `usp_answerproductchangerequest` (raw `NpgsqlCommand`, named params, NOT the positional `CreateCommandWithStoredProcedure`).
- **Proc repo state (changed 2026-07-27):** the two READ procs are **now committed** — `179_usp_GetSecretaryCompetitionChangeRequests.sql` (list) and `180_usp_GetHostSecretaryPendingChangeRequestsCount.sql` (count), captured verbatim from live incl. the CAP-1 filter. The two **ANSWER procs remain LIVE-ONLY / uncommitted** (`usp_answerchangeentryrequest`, `usp_answerproductchangerequest`) — still read them live via Supabase MCP, do not trust `RideOnDB/` for those.
  - `usp_getsecretarycompetitionchangerequests(p_competitionid, p_ranchid, p_status='Pending')` — UNION of entry+product requests. Returns **20 PascalCase columns** (`RequestId, RequestSource, RequestType, CompetitionId, CompetitionName, RequestDate, RequestedByPersonId, RequestedByName, EntityType, EntityName, BeforeText, AfterText, Status, IsCancelled, OriginalEntityId, NewEntityId, FineId, FineAmountSnapshot, AmountBefore, AmountAfter`). `BeforeText`/`AfterText` are `|`-delimited `label: value` strings (incl. a `חיוב:` money part and, for stalls, a `תאריכים: DD/MM/YYYY - DD/MM/YYYY` range). **No `IsPaid`/`CanAnswer` flag.** `RequestType` values are localized: `'שינוי מקצה'`/`'ביטול מקצה'` (entry), `'שינוי מוצר'`/`'ביטול מוצר'` (product) — display-only, no longer used for filtering.
  - `usp_gethostsecretarypendingchangerequestscount(p_ranchid)` — ranch-wide pending count (badge "בקשות ממתינות בכל תחרויות החווה"); note it is ranch-wide while the list is competition-scoped (badge can legitimately exceed one competition's visible rows — by design, not a bug).
  - `usp_answerchangeentryrequest` / `usp_answerproductchangerequest(p_id, p_answerstatus, p_answeredbysystemuserid, p_notes)` — approve/reject. Approve mutates `billcharge` and (entry) **auto-inserts a Fine billcharge** via an `effective_fine` lookup (`EntryCancellation`/`LateRegistration`, `triggermode` Between/After). Entry cancellation `AmountAfter`: 0 before/at registration-end; fine-only between reg-end and start; **full charge (= `AmountBefore`) on/after competition start** (the CAP-10 "post-start full charge kept" branch). Product cancellation `AmountAfter` is always 0.
- **Core business rule (`P0001`, verified 2026-07-27):** if a `billcharge` for the source is `chargestatus='Paid'`, the answer proc `RAISE`s `'Cannot answer change request for a paid ... request'` — the guard sits **above** the approve/reject split, so it blocks **both** (a paid request would be an un-clearable Pending zombie, 400 on every click). **CAP-1 now hides these:** both READ procs carry a Pending-scoped `not exists(paid billcharge)` mirroring the answer procs' exact paid definition (`chargestatus='Paid'`, `sourcetype='Entry'`/`sourceid=originalentryid`, or `'ProductRequest'`/`originalprequestid`). Answered history is preserved; the `P0001` guards stay as backstop; CAP-4 maps any residual leak to Hebrew so `P0001` never reaches the UI. Applied+verified live 2026-07-27 (comp7/ranch11 product-change #4: list 2→1, badge 4→3).
- **Frontend implementation (all 11 CAPs, commit `a260cf9`):** type filter keys off `IsCancelled` (`change`/`cancel`), not the localized `RequestType` string (CAP-2, fixes product requests vanishing). Shared `utils/changeTracking.utils.js` = status/source Hebrew labels + `getAnswerErrorMessage` (CAP-4) + `isPostStartFullChargeCancellation`/`POST_START_FULL_CHARGE_LABEL` (CAP-10). New `change-tracking/DateRangeText.jsx` isolates `DD/MM/YYYY` ranges LTR under RTL (CAP-9). Approve **and** Reject both open the shared `superuser/ConfirmDialog` (CAP-7). Per-action spinner via `answeringAction` (CAP-8). Two distinct empty states (CAP-6). Summary consolidated to total + change + cancel, type axis (CAP-11). Cancellation compact-summary uses the same `סכום: {before} ← {after}` format as changes; fine line reads `החיוב הסופי כולל קנס: ₪X` (Oren refinement 2026-07-27).
- **Frontend cargo (still present, deferred cleanup):** API returns consistent **PascalCase**, but hook/table/modal still defend against both cases via `getValue(item, camelKey, pascalKey)` — camelCase half is dead code. `buildChangedFields` still duplicated across `ChangeRequestsTable.jsx` + `ChangeRequestDetailsModal.jsx`.
- **Still open:** live browser walkthrough (page is login-gated, Claude can't auth — use Claude-in-Chrome on Oren's session or a `_devtest` harness). Deferred (in `_bmad-output/change-tracking-followups.md`): block-at-mobile-creation, refund engine, the dead-code cleanups above.
...
**The three "delete" procs (`usp_deleteserviceproduct`, `usp_deletecompound`, `usp_deletearena`) are LIVE-ONLY — they were never in the repo.** Corrected `usp_deleteserviceproduct` now has repo source at `RideOnDB/StoredProcedures/PostgreSQL/Individual/181_usp_DeleteServiceProduct.sql`.
