...
- **The implementation does NOT do this as of 2026-07-21 — QA iss 40.** `computeDaySchedule` threads a **single** cursor across every class in the day (`classSchedule.utils.js:275`, `cursor = finishMinutes` per iteration), so tied classes are **serialized**: a 5-class tie is charged five durations plus four between-class gaps instead of one. `resolveDayOrigin` resolves the tie for the ORIGIN only; nothing downstream knows ties exist. The predicted day-end is inflated silently — a plausible clock time, no null, dash, or warning. Worst on reining, where 5-class ties are routine, and reining is not exempt.
...
- **Open at time of writing:** the tied-`orderInDay` serialization defect above (**iss 40**, highest priority — it is live and silent); the null-`orderInDay` behaviour is **undecided and blocks implementation of iss 40** (`orderinday` is a NULL-able smallint and `sortClasses` coerces null to 0, so every null-order class in a day collapses into ONE tied position and would be treated as simultaneous — live data has this: comp 45 day 2026-07-07 has 4 of 5 classes with null `orderinday` across 3 arenas; the same weak field already sits behind the tied-group write note above); the reining flattening model is not yet implemented (gap is still applied uniformly to every field); the 06:00 suggestion floor clamps silently with no text when the clamped suggestion no longer resolves the overage; and the `classDateTime` timestamptz round-trip through the apply button is unverified.

#### Classes page view structure (decided 2026-07-21 — supersedes the earlier "expandable column group" idea)
...
- **Only financial columns move out.** Every non-financial column stays in ALL views.
- **The schedule appears in ALL views**, with a different purpose in each: forecasting before the competition, "when and where is every class" during it. It does NOT retire when the recommendations do — advice retires, arithmetic does not. Day two of a three-day competition still needs a correct day-two finish time.
- **Default tab derives from `competition.registrationenddate`.** The secretary never picks.
- **No greying out.** An unavailable tab states what it is waiting for ("available once registration closes"), never reads as disabled.
...
- Warning text must not be crammed into table cells at `text-[10px]` (the current assumed-start caveat is 40 Hebrew words rendered that way, on the first row of every day — effectively unreadable).

**Registration-open window (iss 42, parked).** Between `registrationopendate` and `registrationenddate`, actual-vs-predicted is a live instrument answering three questions at once: is the competition filling, is the prediction model accurate, should marketing push now. It must be denominated by **position in the window**, not raw percentage — 60% of predicted on day 2 of a 30-day window is healthy, 60% with 3 days left is urgent. Copy caution: early in the window the *prediction* is usually the less reliable side, so wording leans "the forecast may be high" over "the competition is underperforming."
