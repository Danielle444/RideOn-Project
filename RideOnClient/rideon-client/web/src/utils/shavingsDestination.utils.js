...
// human-readable "מתחם X · תא/תאים Y" lines the secretary reads. Pure, no React, no DOM —
// deliberately kept portable so the RanchWorker mobile card can reuse it unchanged instead
// of growing a second implementation (see the task brief: "do not duplicate destination
// formatting in Secretary and Worker").
//
...
export function getDeliveryDestinations(order) {
  const value = getValue(order, "deliveryDestinations", "DeliveryDestinations", []);
  return Array.isArray(value) ? value : [];
}

export function getHasUnassignedStalls(order) {
  return (
    getValue(order, "hasUnassignedStalls", "HasUnassignedStalls", false) === true
  );
}
