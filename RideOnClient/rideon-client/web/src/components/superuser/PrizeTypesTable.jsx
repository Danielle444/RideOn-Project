...
export default function PrizeTypesTable(props) {
  const rows = Array.isArray(props.prizeTypes) ? props.prizeTypes : [];
    ...
    <DataTableShell tableClassName="w-full min-w-[980px] text-right">
      <thead>
        <tr className="border-b border-[#E8DDD6] bg-[#FAF7F5] text-sm text-[#6A5248]">
          <th className="px-5 py-4 font-bold">שם פרס</th>
      ...
      <tbody>
        {props.loading && <DataTableLoadingState colSpan={3} />}

        {!props.loading && rows.length === 0 && (
          <DataTableEmptyState colSpan={3} message="לא קיימים פרסים להצגה" />
        )}
        ...
        {!props.loading &&
          rows.map(function (item, index) {
            return (
              <tr
                key={item.prizeTypeId}
                className={
                  "border-b border-[#F1E8E3] text-[#3F312B] transition-colors hover:bg-[#FCFAF8] " +
                  (index % 2 === 0 ? "bg-white" : "bg-[#FFFEFD]")
                }
              >
                <td className="px-5 py-5 font-medium">{item.prizeTypeName}</td>
                <td className="px-5 py-5">{item.prizeDescription || "-"}</td>
