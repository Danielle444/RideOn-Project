...
export default function ArenasTable(props) {
      ...
      <DataTableShell widthMode="full">
        <thead className="bg-[#FAF7F5]">
          <tr className="border-b border-[#E8DDD6] text-sm text-[#6A5248]">
            <th className="px-5 py-4 font-bold text-right">שם מגרש</th>
            <th className="px-5 py-4 font-bold text-center">אורך (מ׳)</th>
            <th className="px-5 py-4 font-bold text-center">רוחב (מ׳)</th>
            <th className="px-5 py-4 font-bold text-center">מקורה</th>
        ...
        <tbody>
          ...
          {!props.loading && rows.length === 0 && (
            <DataTableEmptyState colSpan={5} message="לא קיימים מגרשים להצגה" />
          )}
          ...
          {!props.loading &&
            rows.map(function (item, index) {
              return (
                ...
                >
                  <td className="px-5 py-5 font-medium">{item.arenaName}</td>
                  <td className="px-5 py-5 text-center">{item.arenaLength ?? "—"}</td>
                  <td className="px-5 py-5 text-center">{item.arenaWidth ?? "—"}</td>
                  <td className="px-5 py-5 text-center">
                    <span className="inline-flex rounded-lg bg-[#F3EEEA] px-3 py-1 text-sm font-semibold text-[#6B574F]">
                      {formatCovered(item.isCovered)}
                    </span>
                  </td>
